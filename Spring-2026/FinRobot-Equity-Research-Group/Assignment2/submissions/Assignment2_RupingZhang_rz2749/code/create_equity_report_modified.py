...
# We add this line to insert our "generate_text_section" function
    ...
    if text_type in ["competitor_analysis", "major_takeaways"] and api_key:
        try:
            ...
            try:
                ...
                print(f"❌ Original FinRobot text generator is not available: {e}")
                print("Using validated original text content instead.")
...
def process_text_content(args, analysis_df, peer_ebitda_df, peer_ev_ebitda_df, openai_api_key):
    ...
    for text_type, raw_content in raw_texts.items():
        print(f"📝 Processing {text_type}...")
                    #!!! Assignment 2 enhancement: use Analyst -> Critic -> Reviser
                    # before the original FinRobot text-processing logic.
        #!!! We revise this part
        if getattr(args, "enable_assignment2_enhancement", False):
            # 这里就是我们刚刚从顶部移下来的“延迟导入”
            from modules.assignment2_text_enhancer import enhance_text_section_with_acr
            #!!! We add a tag here to denote which model we use
            print(f"🚀 Assignment 2 ACR enhancement enabled for section: {text_type}")

            analysis_summary = analysis_df.head(20).to_string(index=False)

            processed_texts[text_type] = enhance_text_section_with_acr(
                text_type=text_type,
            ...
            # We made some adjustment here
            if text_type == "tagline":
                processed_texts[text_type] = (
                    "Assignment 2 ACR Enhanced Report "
                    "(Analyst=Claude, Critic=GPT/OpenAI, Reviser=Claude). "
                    + processed_texts[text_type]
                    )
...
def main():
    ...
    #!!! Assignment 2 enhancement option
    parser.add_argument(
    "--enable-assignment2-enhancement",
    action="store_true",
    help="Enable Assignment 2 Claude-inspired Analyst-Critic-Reviser text enhancement."
)
    # 新增增强功能选项
    ...
    report_data = {
        ...
        # We make some adjustment here to present the model we use
        "research_source": (
            args.research_source
            + " | Assignment 2 ACR Enhancement: Analyst=Claude, Critic=GPT/OpenAI, Reviser=Claude"
            if getattr(args, "enable_assignment2_enhancement", False)
            else args.research_source
            ),
