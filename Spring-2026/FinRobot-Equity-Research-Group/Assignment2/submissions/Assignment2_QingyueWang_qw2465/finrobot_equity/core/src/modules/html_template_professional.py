...
def _eu_split_pipe_row(line: str) -> list[str]:
...
def _eu_is_separator_row(cells: list[str]) -> bool:
    if not cells:
        return False
    for c in cells:
        t = (c or "").replace(" ", "")
        if not t or not _re.match(r"^-+$", t):
            return False
    return True


def _eu_render_md_table_html(block_lines: list[str]) -> str:
    rows_raw = [_eu_split_pipe_row(ln) for ln in block_lines if ln.strip()]
    if not rows_raw:
        return ""
    thead_cells: list[str] | None = None
    body_rows = rows_raw
    if len(rows_raw) >= 2 and _eu_is_separator_row(rows_raw[1]):
        thead_cells = rows_raw[0]
        body_rows = rows_raw[2:]

    tbl_style = (
        "border-collapse:collapse;width:100%;font-size:0.8rem;"
        "margin:0.5rem 0;color:#334155;"
    )
    th_style = "border:1px solid #e2e8f0;padding:0.45rem 0.5rem;background:#f8fafc;text-align:left;font-weight:600;"
    td_style = "border:1px solid #e2e8f0;padding:0.45rem 0.5rem;text-align:left;"

    out = [
        f'<div style="overflow-x:auto;margin:0.75rem 0;">',
        f'<table style="{tbl_style}">',
    ]
    if thead_cells is not None:
        out.append("<thead><tr>")
        for c in thead_cells:
            out.append(f"<th style=\"{th_style}\">{_html_escape(c)}</th>")
        out.append("</tr></thead>")
    out.append("<tbody>")
    for row in body_rows:
        out.append("<tr>")
        for c in row:
            out.append(f"<td style=\"{td_style}\">{_html_escape(c)}</td>")
        out.append("</tr>")
    out.append("</tbody></table></div>")
    return "\n".join(out)


def format_earnings_update_html_professional(md_text: str) -> str:
    """将 earnings_update.md（Markdown + pipe 表）转为与专业模板一致的 HTML。"""
    if not md_text or not str(md_text).strip():
        return (
            "<p class='body-text' style='color:#94a3b8;font-style:italic;'>"
            "Earnings Update not available. Run <code>generate_earnings_update.py</code> to create "
            "<code>earnings_update.md</code> in the analysis folder.</p>"
        )

    raw_lines = str(md_text).replace("\r\n", "\n").split("\n")
    while raw_lines and not raw_lines[0].strip():
        raw_lines.pop(0)
    if raw_lines:
        first = raw_lines[0].strip()
        if first.startswith("#") and "earnings update" in first.lower():
    ...
    while i < n:
        ...
        while i < n and raw_lines[i].strip() and not raw_lines[i].strip().startswith("|"):
            ...
            i += 1
        if block:
            chunks.append(("md", block))

    parts: list[str] = []
    for kind, block in chunks:
        if kind == "table":
            parts.append(_eu_render_md_table_html(block))
        else:
            parts.append(_markdown_to_html("\n".join(block)))
    return "\n".join(parts) if parts else (
        "<p class='body-text' style='color:#94a3b8;font-style:italic;'>Earnings Update content empty.</p>"
    )


def build_earnings_update_section_html(include_section: bool, earnings_markdown: str, layout: str) -> str:
    """
    Full HTML for the Earnings Update block (heading + body). Empty when include_section is False
    (baseline model / no Track B in HTML).

    layout: 'professional' | 'combined' | 'page4'
    """
    if not include_section:
        return ""
    body = format_earnings_update_html_professional(earnings_markdown or "")
    if layout == "professional":
        return (
            "\n        <!-- Earnings Update (Track B) -->\n"
            '        <section class="mb-10" id="earnings-update">\n'
            '            <h2 class="section-title">Earnings Update</h2>\n'
            '            <div class="content-card">\n'
            f"{body}\n"
            "            </div>\n"
            "        </section>"
    ...
    if layout == "combined":
        return (
            "\n      <div class=\"mb-6\">\n"
            '        <h2 class="section-label mb-3">Earnings Update</h2>\n'
    ...
    if layout == "page4":
        return (
            "\n      <div class=\"mb-6\">\n"
            '        <h2 style="color: #002855; font-size: 13px; font-weight: 600; text-transform: uppercase; '
            'letter-spacing: 0.04em; border-bottom: 1px solid #d1d5db; padding-bottom: 4px;" class="mb-3">'
            "Earnings Update</h2>\n"
            '        <div class="text-xs leading-tight">'
            f"{body}"
            "</div>\n"
...
def render_professional_html_report(data: dict) -> str:
    ...
    report_data = {
        ...
        'earnings_update_section_html': build_earnings_update_section_html(
            data.get('include_earnings_update_html', True),
            data.get('earnings_update', ''),
            'professional',
        ),
