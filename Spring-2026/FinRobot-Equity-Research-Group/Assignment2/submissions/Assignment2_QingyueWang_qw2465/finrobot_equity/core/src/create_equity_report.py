...
def process_text_content(args, analysis_df, peer_ebitda_df, peer_ev_ebitda_df, openai_api_key, openai_base_url=None, openai_model=None):
...
def main():
    ...
    parser.add_argument(
        "--omit-earnings-update-html",
        action="store_true",
        help="Exclude the Track B Earnings Update block from all HTML outputs (use for baseline model runs).",
    )
    ...
    # - If openai_regen_* is set, regeneration uses those values (mixed model setup).
    # - Otherwise regeneration falls back to openai_* (unified model setup).
    openai_api_key = None
    ...
    try:
        ...
        if args.enable_text_regeneration:
            try:
                primary_api_key = get_api_key(config, "API_KEYS", "openai_api_key")
                primary_base_url = config.get("API_KEYS", "openai_base_url", fallback=None)
                primary_model = config.get("API_KEYS", "openai_model", fallback=None)

                regen_api_key = config.get("API_KEYS", "openai_regen_api_key", fallback=None)
                regen_base_url = config.get("API_KEYS", "openai_regen_base_url", fallback=None)
                regen_model = config.get("API_KEYS", "openai_regen_model", fallback=None)

                # Use regen-specific config if provided; otherwise fall back to primary config.
                openai_api_key = regen_api_key or primary_api_key
                openai_base_url = regen_base_url or primary_base_url
                openai_model = regen_model or primary_model

                print("✅ OpenAI API key loaded for text regeneration")
                if regen_model:
                    print("Using regeneration-specific model config (openai_regen_model)")
                if openai_base_url:
                    print(f"Using OpenAI base URL for regeneration: {openai_base_url}")
                if openai_model:
                    print(f"Using model for regeneration: {openai_model}")
            except Exception as e:
    ...
    report_data = {
        ...
        "retail_sentiment": {},
        "include_earnings_update_html": not getattr(args, "omit_earnings_update_html", False),
    }
    ...
    # Earnings Update markdown (same artifact as PDF; optional)
    analysis_dir_for_html = os.path.dirname(os.path.abspath(args.analysis_csv))
    earnings_update_path = os.path.join(analysis_dir_for_html, "earnings_update.md")
    if os.path.isfile(earnings_update_path):
        try:
            with open(earnings_update_path, "r", encoding="utf-8") as f:
                report_data["earnings_update"] = f.read()
            print(f"✅ Earnings Update loaded for HTML: {earnings_update_path}")
        except Exception as e:
            print(f"⚠️ Error loading earnings_update.md: {e}")
            report_data["earnings_update"] = ""
    else:
        report_data.setdefault("earnings_update", "")
