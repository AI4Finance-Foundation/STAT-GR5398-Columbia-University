...
def generate_text_section(data: Dict, prompt_type: str, api_key: str, company_name: str, company_ticker: str, base_url: str = None, model: str = None) -> str:
    ...
    # High enough for long sections (300–400 words ≈ 500–800 tokens). Some providers
    # (e.g. Gemini with reasoning) charge internal + visible tokens against this cap;
    # a low value yields finish_reason='length' after only a short visible reply.
    max_out = 5000
    ...
    try:
        ...
        if usage is not None:
            ...
            print(f"📎 LLM meta '{prompt_type}': finish_reason={finish!r}, prompt_tokens={pt}, completion_tokens={ct}")
        else:
            print(f"📎 LLM meta '{prompt_type}': finish_reason={finish!r}, usage=(n/a)")

        raw_content = choice.message.content if choice.message else None
        generated_text = (raw_content or "").strip()
