...
class RetailSentimentClient:
    ...
    def get_snapshot(self, ticker: str, days_back: int = 7) -> Dict[str, Any]:
        ...
        for spec in self.PLATFORM_SPECS:
            try:
                ...
                # Light spacing between platforms to reduce burst 429s when running many tickers back-to-back.
                time.sleep(0.35)
    ...
    def _fetch_source_row(
        ...
        # Adanos may return 429 when many tickers run back-to-back; retry with backoff.
        ...
        for attempt in range(len(backoff_seconds)):
            ...
            if response.status_code == 429 and attempt < len(backoff_seconds) - 1:
