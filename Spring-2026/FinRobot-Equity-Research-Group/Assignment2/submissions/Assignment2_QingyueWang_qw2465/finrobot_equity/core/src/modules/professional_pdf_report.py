...
# catalyst_calendar.md 仍可由 generate_catalyst_calendar.py 生成；默认不写入 PDF。
INCLUDE_CATALYST_CALENDAR_IN_PDF = False

# thesis_tracker.md 仍可由 generate_thesis_tracker.py 生成；默认不写入 PDF。
INCLUDE_THESIS_TRACKER_IN_PDF = False
...
class ProfessionalEquityReport:
    ...
    # =========================================================================
    # Earnings Update（Anthropic earnings-analysis skill 子集，Markdown 产物）
    # =========================================================================
    def _earnings_update_split_pipe_row(self, line: str) -> List[str]:
    ...
    def _earnings_update_is_table_sep(self, cells: List[str]) -> bool:
        if not cells:
            return True
        for c in cells:
            t = c.replace(' ', '')
    ...
    def _earnings_update_render_md_table(self, block_lines: List[str]) -> Optional[Table]:
        rows_cells: List[List[str]] = []
        for ln in block_lines:
            cells = self._earnings_update_split_pipe_row(ln)
            if self._earnings_update_is_table_sep(cells):
                continue
        ...
        for ri, row in enumerate(rows_cells):
            row_flow = []
            for c in row:
                txt = clean_markdown_symbols(str(c))
                txt = _xml_escape(txt).replace('\n', ' ')
                ps = hdr_style if ri == 0 else cell_style
                row_flow.append(Paragraph(txt, ps))
            flow.append(row_flow)

        col_w = Layout.CONTENT_WIDTH / max(ncols, 1)
        tbl = Table(flow, colWidths=[col_w] * ncols)
        tbl.setStyle(create_data_table_style())
        return tbl

    def build_earnings_update_section(self):
        """构建 Earnings Update 部分（读取 earnings_update.md；Markdown 表格转为 PDF Table）"""
        body = self.data.get('earnings_update', '')
        if not body or not str(body).strip():
            return

        raw_lines = str(body).split('\n')

        self.elements.append(Spacer(1, 8 * mm))
        self.elements.append(Paragraph('Earnings Update', self.styles['SectionTitle']))
        self.elements.append(create_separator())
        ...
        while i < len(raw_lines):
            ...
            if s.startswith('|'):
                ...
                while i < len(raw_lines) and raw_lines[i].strip().startswith('|'):
                    ...
                    i += 1
                tbl = self._earnings_update_render_md_table(block)
                if tbl:
                    self.elements.append(tbl)
                    self.elements.append(Spacer(1, 3 * mm))
                continue

            cleaned = clean_markdown_symbols(s)

            if cleaned.startswith('#'):
                title = re.sub(r'^#+\s*', '', cleaned).strip()
                if title and 'Earnings Update' not in title:
            ...
            if cleaned.startswith('*') and cleaned.endswith('*') and len(cleaned) > 2:
                ...
                self.elements.append(Paragraph(f'<i>{_xml_escape(inner)}</i>', self.styles['Body']))
                i += 1
                continue

            self.elements.append(Paragraph(cleaned, self.styles['Body']))
            i += 1
    ...
    # Thesis Tracker 部分
    # =========================================================================
    def build_thesis_tracker_section(self):
        """构建 Thesis Tracker 部分（读取 thesis_tracker.md 的结构化要点）"""
        tracker = self.data.get('thesis_tracker', '')
        if not tracker or not str(tracker).strip():
            return

        # 清理 markdown 表格/分隔线，仅保留章节标题与正文要点
        lines = []
        for line in str(tracker).split('\n'):
            s = line.strip()
            if not s:
                lines.append("")
                continue
        ...
        cleaned = clean_markdown_symbols("\n".join(lines))
        if not cleaned.strip():
            return

        self.elements.append(Spacer(1, 8 * mm))
        self.elements.append(Paragraph("Thesis Tracker", self.styles['SectionTitle']))
        self.elements.append(create_separator())
        ...
        for raw in cleaned.split('\n'):
            ...
            if line.startswith("#"):
                ...
                if title and "Thesis Tracker" not in title:
                    self.elements.append(Paragraph(title, self.styles['Heading3']))
                continue
            ...
            self.elements.append(Paragraph(line, self.styles['Body']))

    # =========================================================================
    # Catalyst Calendar 部分（时间轴 + 类型，独立于叙事版 Key Catalysts）
    # =========================================================================
    def build_catalyst_calendar_section(self):
        """构建 Catalyst Calendar 部分（读取 catalyst_calendar.md）"""
        cal = self.data.get('catalyst_calendar', '')
        if not cal or not str(cal).strip():
            return
        ...
        for line in str(cal).split('\n'):
            ...
            if not s:
                lines_out.append("")
                continue
            if s.startswith('|') or (s.startswith('---') and len(s) >= 3 and set(s) <= {'-', ' '}):
                continue
        ...
        self.elements.append(Spacer(1, 8 * mm))
        self.elements.append(Paragraph('Catalyst Calendar', self.styles['SectionTitle']))
        self.elements.append(create_separator())
        ...
        for raw in cleaned.split('\n'):
            ...
            if line.startswith('#'):
                title = re.sub(r'^#+\s*', '', line).strip()
                if title and 'Catalyst Calendar' not in title:
                    self.elements.append(Paragraph(title, self.styles['Heading3']))
                continue
    ...
    def build(self) -> str:
        ...
        self.build_earnings_update_section()     # Earnings Update — earnings-analysis skill subset (Track B)
        if INCLUDE_CATALYST_CALENDAR_IN_PDF:
            self.build_catalyst_calendar_section()
        self.build_catalyst_section()            # 催化剂分析 (新增)
        if INCLUDE_THESIS_TRACKER_IN_PDF:
            self.build_thesis_tracker_section()
