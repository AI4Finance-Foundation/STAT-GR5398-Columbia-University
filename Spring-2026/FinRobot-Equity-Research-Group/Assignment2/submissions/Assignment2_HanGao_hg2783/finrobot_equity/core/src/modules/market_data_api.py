...
# --- Helper functions for FMP API fallback and DataFrame limiting ---

def _fetch_fmp_json_with_fallback(stable_url: str, legacy_url: str | None = None, timeout: int = 30):
    """Try FMP stable endpoint first, then fall back to legacy endpoint if provided."""
    last_error = None

    for url in [stable_url, legacy_url]:
        if not url:
            continue
        try:
...
def _limit_fmp_dataframe(df: pd.DataFrame, limit: int) -> pd.DataFrame:
    """Sort newest-first by date if available and keep the requested number of rows."""
...
def get_fmp_ratios_and_key_metrics(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    ...
    try:
        # Ratios
        stable_ratios_url = f"https://financialmodelingprep.com/stable/ratios?symbol={ticker}&apikey={api_key}"
        legacy_ratios_url = f"https://financialmodelingprep.com/api/v3/ratios/{ticker}?period={period}&limit={limit}&apikey={api_key}"
        ratios_data = _fetch_fmp_json_with_fallback(stable_ratios_url, legacy_ratios_url)
        if ratios_data:
            ratios_df = pd.DataFrame(ratios_data)
            if "date" in ratios_df.columns:
                ratios_df["date"] = pd.to_datetime(ratios_df["date"])
                ratios_df["year"] = ratios_df["date"].dt.year
            ratios_df = _limit_fmp_dataframe(ratios_df, limit)

        # Key Metrics
        stable_key_metrics_url = f"https://financialmodelingprep.com/stable/key-metrics?symbol={ticker}&apikey={api_key}"
        legacy_key_metrics_url = f"https://financialmodelingprep.com/api/v3/key-metrics/{ticker}?period={period}&limit={limit}&apikey={api_key}"
        key_metrics_data = _fetch_fmp_json_with_fallback(stable_key_metrics_url, legacy_key_metrics_url)
        if key_metrics_data:
            key_metrics_df = pd.DataFrame(key_metrics_data)
            if "date" in key_metrics_df.columns:
                key_metrics_df["date"] = pd.to_datetime(key_metrics_df["date"])
                key_metrics_df["year"] = key_metrics_df["date"].dt.year
            key_metrics_df = _limit_fmp_dataframe(key_metrics_df, limit)
...
def get_fmp_income_statement(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> pd.DataFrame | None:
    ...
    try:
        stable_url = f"https://financialmodelingprep.com/stable/income-statement?symbol={ticker}&apikey={api_key}"
        legacy_url = f"https://financialmodelingprep.com/api/v3/income-statement/{ticker}?period={period}&limit={limit}&apikey={api_key}"
        data = _fetch_fmp_json_with_fallback(stable_url, legacy_url)
        if not data:
        ...
        df = _limit_fmp_dataframe(df, limit)
        return df
    except requests.exceptions.RequestException as e:
...
def get_fmp_balance_sheet(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> pd.DataFrame | None:
    ...
    try:
        stable_url = f"https://financialmodelingprep.com/stable/balance-sheet-statement?symbol={ticker}&apikey={api_key}"
        legacy_url = f"https://financialmodelingprep.com/api/v3/balance-sheet-statement/{ticker}?period={period}&limit={limit}&apikey={api_key}"
        data = _fetch_fmp_json_with_fallback(stable_url, legacy_url)
        if not data:
...
def get_fmp_cash_flow_statement(ticker: str, api_key: str, period: str = "annual", limit: int = 5) -> pd.DataFrame | None:
    ...
    try:
        stable_url = f"https://financialmodelingprep.com/stable/cash-flow-statement?symbol={ticker}&apikey={api_key}"
        legacy_url = f"https://financialmodelingprep.com/api/v3/cash-flow-statement/{ticker}?period={period}&limit={limit}&apikey={api_key}"
        data = _fetch_fmp_json_with_fallback(stable_url, legacy_url)
        if not data:
...
def combine_peer_financial_data(tickers: list[str], api_key: str, years_limit: int = 5) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    More robust version:
    1. Pulls EBITDA from income statement.
    2. Pulls EV/EBITDA from key metrics if available.
    3. If direct EV/EBITDA is unavailable, computes EV/EBITDA = Enterprise Value / EBITDA when enterprise value exists.
    """

    def _get_year(row):
        if "year" in row and pd.notna(row.get("year")):
            return int(row.get("year"))
        if "fiscalYear" in row and pd.notna(row.get("fiscalYear")):
            return int(row.get("fiscalYear"))
        if "calendarYear" in row and pd.notna(row.get("calendarYear")):
            return int(row.get("calendarYear"))
        if "date" in row and pd.notna(row.get("date")):
            return int(str(row.get("date"))[:4])
        return None

    def _first_valid(row, possible_cols):
    ...
    for ticker in tickers:
        ...
        # 1. EBITDA from income statement
        if income_df is not None and not income_df.empty:
            for _, row in income_df.iterrows():
                ...
                if year not in ticker_data:
                    ticker_data[year] = {}

                ebitda = _first_valid(row, ["ebitda", "EBITDA"])
                if ebitda is not None:
                    ticker_data[year]["EBITDA"] = ebitda

        # 2. EV / EBITDA from key metrics, or compute from EV and EBITDA
        if key_metrics_df is not None and not key_metrics_df.empty:
            for _, row in key_metrics_df.iterrows():
                ...
                # Different FMP endpoints use different names
                ...
                # If direct EV/EBITDA unavailable, compute using Enterprise Value / EBITDA
                enterprise_value = _first_valid(
                    row,
                    [
                        "enterpriseValue",
                        "enterpriseValueTTM",
                ...
                ebitda = ticker_data.get(year, {}).get("EBITDA")

                if ev_ebitda is None and enterprise_value is not None and ebitda not in [None, 0]:
                    try:
                        ev_ebitda = float(enterprise_value) / float(ebitda)
                    except Exception:
                        ev_ebitda = None

                if ev_ebitda is not None:
                    ticker_data[year]["EV/EBITDA"] = ev_ebitda
        ...
        for year, metrics in yearly_data.items():
            if "EBITDA" in metrics and metrics["EBITDA"] is not None:
                ebitda_records.append(
                    {"ticker": ticker, "year": year, "EBITDA": metrics["EBITDA"]}
                )
    ...
    if not df_ebitda_all.empty:
        df_ebitda_pivot = (
            df_ebitda_all
            .pivot(index="year", columns="ticker", values="EBITDA")
            .sort_index()
        )
        ...
        for year, metrics in yearly_data.items():
            if "EV/EBITDA" in metrics and metrics["EV/EBITDA"] is not None:
                ev_ebitda_records.append(
                    {"ticker": ticker, "year": year, "EV/EBITDA": metrics["EV/EBITDA"]}
                )
    ...
    if not df_ev_ebitda_all.empty:
        df_ev_ebitda_pivot = (
            df_ev_ebitda_all
            .pivot(index="year", columns="ticker", values="EV/EBITDA")
            .sort_index()
        )
