...
class NewsIntegrator:
    ...
    def _parse_news_datetime(value: Any) -> Optional[datetime]:
        """Parse FMP news dates without forcing historical snapshots to current time."""
    ...
    @staticmethod
    def _news_window_anchor(value: Any) -> Optional[datetime]:
        anchor = NewsIntegrator._parse_news_datetime(value)
        if anchor is None:
            return None
        if isinstance(value, str) and len(value.strip()) <= 10:
            return anchor.replace(hour=23, minute=59, second=59, microsecond=999999)
        return anchor

    @staticmethod
    def _news_window_start(anchor: datetime, days: int, date_only_anchor: bool = False) -> datetime:
        if date_only_anchor:
            anchor_start = anchor.replace(hour=0, minute=0, second=0, microsecond=0)
            return anchor_start - timedelta(days=days)
    ...
    def _is_within_time_window(self, published_date: str, days: int = 5, anchor_date: Any = None) -> bool:
        """检查新闻是否在时间窗口内"""
        try:
            pub_date = self._parse_news_datetime(published_date)
            if not pub_date:
                return True  # 无法解析时默认包含

            anchor = self._news_window_anchor(anchor_date) if anchor_date else None
            if anchor is None:
                anchor = datetime.now()

            date_only_anchor = isinstance(anchor_date, str) and len(anchor_date.strip()) <= 10
            cutoff_date = self._news_window_start(anchor, days, date_only_anchor=date_only_anchor)
            return cutoff_date <= pub_date <= anchor
            
        except Exception:
            return True
    
    def process_news(self, days_window: int = 5, anchor_date: Any = None) -> List[NewsArticle]:
        """
        处理和分类新闻
        ...
        for article in self.raw_news:
            ...
            # 检查时间窗口
            if not self._is_within_time_window(published_date, days_window, anchor_date=anchor_date):
                continue

            # 过滤无关公司新闻
            if article_symbol != self.ticker.upper() and not self._is_relevant_to_ticker(title, text):
                continue
...
def get_enhanced_company_news(ticker: str, api_key: str, days_back: int = 5,
                              limit: int = 50, as_of_date: str = None) -> Dict[str, Any]:
    """
    增强版公司新闻获取函数
    ...
    """
    try:
        # Reuse market_data_api path so same-day FMP cache policy is applied.
        from .market_data_api import get_company_news
        ...
        integrator.process_news(days_back, anchor_date=as_of_date)
