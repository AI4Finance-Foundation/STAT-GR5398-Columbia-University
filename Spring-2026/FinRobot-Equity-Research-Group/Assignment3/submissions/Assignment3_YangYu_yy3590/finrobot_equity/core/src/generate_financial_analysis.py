...
def main() -> int:
    ...
    parser.add_argument("--as-of-date", type=str, default=None, help="Point-in-time date for visible data (YYYY-MM-DD).")
    ...
    parser.add_argument("--fmp-cache-policy", type=str, default="same-day", choices=["same-day", "reuse-until-forced"], help="FMP cache reuse policy.")
    parser.add_argument("--force-refresh-fmp", action="store_true", help="Bypass FMP cache and refresh from API.")
    parser.add_argument("--fmp-cache-dir", type=str, default=None, help="Optional directory for FMP cache files.")
    parser.add_argument("--llm-provider", type=str, default=None, help="Override LLM provider for text generation.")
    parser.add_argument("--llm-model", type=str, default=None, help="Override LLM model for text generation.")
    ...
    configure_fmp_runtime(
        ...
        cache_policy=args.fmp_cache_policy,
        clear_events=True,
    )
    ...
    if args.peer_tickers and fmp_api_key:
        ...
        try:
            ...
            df_ebitda_peers, df_ev_ebitda_peers = combine_peer_financial_data(
                all_tickers, fmp_api_key, years_limit=args.years_limit, as_of_date=args.as_of_date
            )
