...
from modules.equity_agents.revenue_driver_analyzer import RevenueDriverAnalyzer
...
from modules.equity_agents.earnings_analyzer import EarningsAnalyzer
...
def main():
    ...
    # Text Generation
    parser.add_argument("--generate-text-sections", action="store_true", help="Enable generation of text sections using OpenAI, Anthropic, or Gemini.")
    parser.add_argument("--text-output-dir", type=str, default=None, help="Directory to save generated text files.")
    parser.add_argument("--llm-provider", type=str, default=None, choices=["openai", "anthropic", "gemini"], help="LLM provider for text generation: openai, anthropic, or gemini. Defaults to config value or openai.")
    parser.add_argument("--llm-model", type=str, default=None, help="Optional model override for text generation.")
    ...
    #  SEC filing
    parser.add_argument("--enable-sec-filing-analysis", action="store_true", help="Enable SEC filing analysis")
    parser.add_argument("--sec-filing-types", type=str, nargs="*", default=["10-K", "10-Q"], help="SEC filing types to analyze")
    parser.add_argument("--sec-filing-limit", type=int, default=2, help="Maximum number of SEC filings to analyze")
    parser.add_argument("--enable-revenue-driver-analysis", action="store_true", help="Enable revenue driver analysis")
    ...
    try:
        ...
        if args.generate_text_sections:
            ...
            try:
                ...
                llm_provider = "openai"
            llm_provider = (llm_provider or "openai").lower().strip()

            if llm_provider == "anthropic":
                text_api_key = get_api_key(config, section="ANTHROPIC", key="anthropic_api_key")
                text_model = args.llm_model or get_api_key(config, section="ANTHROPIC", key="anthropic_model")
                text_base_url = None
            elif llm_provider == "gemini":
                text_api_key = get_api_key(config, section="GEMINI", key="gemini_api_key")
                text_model = args.llm_model or get_api_key(config, section="GEMINI", key="gemini_model")
                text_base_url = None
            else:
                llm_provider = "openai"
                text_api_key = get_api_key(config, section="API_KEYS", key="openai_api_key")
                try:
                    text_base_url = get_api_key(config, section="API_KEYS", key="openai_base_url")
                    openai_base_url = text_base_url
                    print(f"Using OpenAI base URL: {text_base_url}")
                except Exception:
                    text_base_url = None
                try:
                    text_model = args.llm_model or get_api_key(config, section="API_KEYS", key="openai_model")
                except Exception:
                    text_model = args.llm_model or None
    ...
    # 4.58 SEC Filing Analysis
    sec_filing_results = None
    if args.enable_sec_filing_analysis:
        print(f"\nPerforming SEC filing analysis for {args.company_ticker}...")
        try:
            sec_analyzer = SECFilingAnalyzer()

            sec_filing_results = sec_analyzer.analyze_filings(
                ticker=args.company_ticker,
                company_name=args.company_name,
                filing_types=args.sec_filing_types,
                limit=args.sec_filing_limit
            )

            sec_filing_path = os.path.join(output_dir, "sec_filing_analysis.json")
            with open(sec_filing_path, "w", encoding="utf-8") as f:
                json.dump(sec_filing_results, f, indent=2, ensure_ascii=False, default=str)
            print(f"Saved SEC filing analysis to: {sec_filing_path}")

            sec_filing_summary = sec_filing_results.get("summary", "")
            if sec_filing_summary:
                sec_filing_summary_path = os.path.join(output_dir, "sec_filing_summary.md")
                with open(sec_filing_summary_path, "w", encoding="utf-8") as f:
                    f.write(sec_filing_summary)
                print(f"Saved SEC filing summary to: {sec_filing_summary_path}")

        except Exception as e:
            print(f"Error performing SEC filing analysis: {e}")
            print("Continuing without SEC filing analysis...")
    else:
        print("Skipping SEC filing analysis (not enabled)")

    # 4.59 Revenue Driver Analysis
    revenue_driver_results = None
    if args.enable_revenue_driver_analysis:
        print(f"\nPerforming revenue driver analysis for {args.company_ticker}...")
        try:
            revenue_driver_analyzer = RevenueDriverAnalyzer()

            forecast_assumptions = {
                2025: args.revenue_growth_2025,
                2026: args.revenue_growth_2026,
                2027: args.revenue_growth_2027,
            }

            revenue_driver_results = revenue_driver_analyzer.analyze(
                ticker=args.company_ticker,
                company_name=args.company_name,
                financial_metrics_df=final_data_df,
                company_news=company_news,
                sec_filing_analysis=sec_filing_results,
                forecast_assumptions=forecast_assumptions,
            )

            revenue_driver_path = os.path.join(output_dir, "revenue_driver_analysis.json")
            with open(revenue_driver_path, "w", encoding="utf-8") as f:
                json.dump(revenue_driver_results, f, indent=2, ensure_ascii=False, default=str)

            print(f"Saved revenue driver analysis to: {revenue_driver_path}")

        except Exception as e:
            print(f"Error performing revenue driver analysis: {e}")
            print("Continuing without revenue driver analysis...")
    else:
        print("Skipping revenue driver analysis (not enabled)")

    # 4.65 Earnings Analysis
    earnings_analysis_results = None
    print(f"\nPerforming earnings analysis for {args.company_ticker}...")
    try:
        earnings_analyzer = EarningsAnalyzer()
        earnings_analysis_results = earnings_analyzer.analyze(
            ticker=args.company_ticker,
            company_name=args.company_name,
            analysis_df=final_data_df,
            peer_ebitda_df=projected_peer_ebitda,
            peer_ev_ebitda_df=df_ev_ebitda_peers,
        )

        earnings_path = os.path.join(output_dir, "earnings_analysis.json")
        with open(earnings_path, "w", encoding="utf-8") as f:
            json.dump(earnings_analysis_results, f, indent=2, ensure_ascii=False, default=str)
        print(f"Saved earnings analysis to: {earnings_path}")

        earnings_summary_path = os.path.join(output_dir, "earnings_summary.md")
        with open(earnings_summary_path, "w", encoding="utf-8") as f:
            f.write(earnings_analysis_results.get("summary", "") + "\n\n")
            f.write("Key Earnings Read-Through\n")
            for point in earnings_analysis_results.get("key_points", []):
                f.write(f"- {point}\n")
            f.write("\nKey Watch Items\n")
            for item in earnings_analysis_results.get("watch_items", []):
                f.write(f"- {item}\n")

        print(f"Saved earnings summary to: {earnings_summary_path}")

    except Exception as e:
        print(f"Warning: Earnings analysis failed: {e}")
        earnings_analysis_results = None
    ...
    if args.generate_text_sections:
        ...
        if not text_api_key:
            print(f"Error: API key for provider '{llm_provider}' not loaded. Skipping text generation.")
        else:
            data_for_text_gen = {
                ...
                "sec_filing_analysis": sec_filing_results,
                "revenue_driver_analysis": revenue_driver_results,
    ...
    summary_data = {
        ...
        "llm_provider": llm_provider if args.generate_text_sections else None,
        "llm_model": text_model if args.generate_text_sections else None,
        "text_generation_enabled": args.generate_text_sections,
        ...
        "sec_filing_analysis_enabled": args.enable_sec_filing_analysis,
        "sec_filing_types": args.sec_filing_types,
        "sec_filing_limit": args.sec_filing_limit,
        "revenue_driver_analysis_enabled": args.enable_revenue_driver_analysis,
        ...
        "files_generated": {
            ...
            "sec_filing_analysis": "sec_filing_analysis.json" if sec_filing_results else None,
            "revenue_driver_analysis": "revenue_driver_analysis.json" if revenue_driver_results else None,
            ...
            "earnings_analysis": "earnings_analysis.json" if earnings_analysis_results else None,
            "earnings_summary": "earnings_summary.md" if earnings_analysis_results else None,
