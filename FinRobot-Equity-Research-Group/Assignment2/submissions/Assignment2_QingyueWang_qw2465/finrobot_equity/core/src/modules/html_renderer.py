...
def render_html_report(template_str: str, data: dict) -> str:
    ...
    try:
        ...
        try:
            from modules.html_template_professional import build_earnings_update_section_html

            inc = data.get("include_earnings_update_html", True)
            data["earnings_update_page4_block"] = build_earnings_update_section_html(
                inc, data.get("earnings_update", ""), "page4"
            )
        except Exception as e:
            print(f"Warning: Error formatting earnings update: {e}")
            data["earnings_update_page4_block"] = ""
...
<body class="bg-white text-xs">
  <div class="page-container mx-auto">
    ...
    <section id="sensitivity" class="section page-break">
      ...
      {earnings_update_combined_block}
      <div class="mb-6">
...
def render_combined_html_report(data: dict) -> str:
    ...
    try:
        ...
        try:
            ...
            inc = data.get("include_earnings_update_html", True)
            data["earnings_update_combined_block"] = build_earnings_update_section_html(
                inc, data.get("earnings_update", ""), "combined"
            )
        except Exception as e:
            print(f"Warning: Error formatting earnings update: {e}")
            data["earnings_update_combined_block"] = ""
