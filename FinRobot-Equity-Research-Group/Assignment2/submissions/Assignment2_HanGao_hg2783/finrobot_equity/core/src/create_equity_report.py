...
def _format_bullets_html(items, max_items: int = 4) -> str:
    if not items:
        return ""
    clean_items = []
    ...
    if not clean_items:
        return ""
    bullets = "".join(
        f'<li style="padding-left:10px; margin-bottom:2px; font-size:11px; color:#1a1a1a;">&#9642; {html.escape(item)}</li>'
        for item in clean_items
    )
    return f'<ul style="list-style:none; padding-left:0; margin:3px 0;">{bullets}</ul>'


def build_sec_filing_highlights_html(sec_filing_data: dict) -> str:
    if not sec_filing_data:
        return ""

    filings = sec_filing_data.get("filings_analyzed", []) or []
    filing_lines = []
    key_takeaways = []
    risk_signals = []
    catalyst_signals = []
    ...
    for filing in filings[:2]:
        ...
        filing_lines.append(f"{filing_type} dated {filing_date}")
        key_takeaways.extend(filing.get("key_takeaways", []) or [])
        risk_signals.extend(filing.get("risk_signals", []) or [])
        catalyst_signals.extend(filing.get("catalyst_signals", []) or [])

    filing_meta_html = ""
    if filing_lines:
        filing_meta_html = (
            '<p style="margin-bottom:6px; font-size:11px; color:#555555;">'
            f'Recent filings reviewed: {html.escape("; ".join(filing_lines))}</p>'
        )

    summary = sec_filing_data.get("summary", "")
    summary_html = ""
    if summary:
        summary_html = (
            '<p style="margin-bottom:6px; font-size:11px; color:#1a1a1a;">'
            f"{html.escape(str(summary))}</p>"
        )

    takeaways_html = _format_bullets_html(key_takeaways, max_items=4)
    risks_html = _format_bullets_html(risk_signals, max_items=4)
    catalysts_html = _format_bullets_html(catalyst_signals, max_items=4)

    if not any([filing_meta_html, summary_html, takeaways_html, risks_html, catalysts_html]):
        return ""

    parts = [
        '<h3 class="section-label">SEC Filing Highlights</h3>',
        '<div class="mb-4 text-xs leading-tight" style="color: #1a1a1a;">',
        filing_meta_html,
        summary_html,
    ]
    if takeaways_html:
        parts.append('<p style="margin-bottom:4px; font-size:11px; color:#002855; font-weight:600;">Key Takeaways</p>')
        parts.append(takeaways_html)
    if risks_html:
        parts.append('<p style="margin:8px 0 4px 0; font-size:11px; color:#002855; font-weight:600;">Risk Signals</p>')
        parts.append(risks_html)
    if catalysts_html:
        parts.append('<p style="margin:8px 0 4px 0; font-size:11px; color:#002855; font-weight:600;">Catalyst Signals</p>')
        parts.append(catalysts_html)
    parts.append("</div>")
    return "".join(parts)


def build_revenue_driver_analysis_html(revenue_driver_data: dict) -> str:
    if not revenue_driver_data:
        return ""

    historical = revenue_driver_data.get("historical_revenue_analysis", {}) or {}
    historical_trend = historical.get("historical_growth_trend")
    average_growth = historical.get("average_growth")
    observations = historical.get("observations", []) or []
    driver_summary = revenue_driver_data.get("driver_summary", []) or []
    forecast_support = revenue_driver_data.get("forecast_support", {}) or {}
    summary = revenue_driver_data.get("summary", "")

    intro_lines = []
    if historical_trend:
        intro_lines.append(f"Historical revenue trend: {historical_trend}.")
    if average_growth is not None:
        try:
            intro_lines.append(f"Average historical revenue growth: {float(average_growth):.1%}.")
        except Exception:
            intro_lines.append(f"Average historical revenue growth: {average_growth}.")
    if summary:
        intro_lines.append(str(summary))

    intro_html = ""
    if intro_lines:
        intro_html = (
            '<p style="margin-bottom:6px; font-size:11px; color:#1a1a1a;">'
            f"{html.escape(' '.join(intro_lines))}</p>"
        )

    observations_html = _format_bullets_html(observations, max_items=3)
    drivers_html = _format_bullets_html(driver_summary, max_items=5)
    forecast_lines = [f"{year}: {detail}" for year, detail in forecast_support.items()]
    forecast_html = _format_bullets_html(forecast_lines, max_items=3)

    if not any([intro_html, observations_html, drivers_html, forecast_html]):
        return ""

    parts = [
        '<h3 class="section-label">Revenue Driver Analysis</h3>',
        '<div class="mb-4 text-xs leading-tight" style="color: #1a1a1a;">',
        intro_html,
    ]
    if observations_html:
        parts.append('<p style="margin-bottom:4px; font-size:11px; color:#002855; font-weight:600;">Historical Observations</p>')
        parts.append(observations_html)
    if drivers_html:
        parts.append('<p style="margin:8px 0 4px 0; font-size:11px; color:#002855; font-weight:600;">Identified Revenue Drivers</p>')
        parts.append(drivers_html)
    if forecast_html:
        parts.append('<p style="margin:8px 0 4px 0; font-size:11px; color:#002855; font-weight:600;">Forecast Support</p>')
        parts.append(forecast_html)
    parts.append("</div>")
    return "".join(parts)


def build_earnings_analysis_html(earnings_summary_file: str) -> str:
    """Load earnings_summary.md and convert it into a compact HTML section for the report."""
    if not earnings_summary_file or not os.path.exists(earnings_summary_file):
        return ""

    try:
        with open(earnings_summary_file, "r", encoding="utf-8") as f:
        ...
        parts = [
            '<h3 class="section-label">Earnings Analysis</h3>',
            '<div class="mb-4 text-xs leading-tight" style="color: #1a1a1a;">'
        ]
        ...
        for line in lines:
            ...
            if line in ["Key Earnings Read-Through", "Key Watch Items"]:
                if in_list:
                    parts.append("</ul>")
                    in_list = False
                parts.append(
                    f'<p style="margin:8px 0 4px 0; font-size:11px; color:#002855; font-weight:600;">{html.escape(line)}</p>'
                )
                parts.append('<ul style="list-style:none; padding-left:0; margin:3px 0;">')
                in_list = True
                continue
            ...
            parts.append(
                f'<p style="margin-bottom:6px; font-size:11px; color:#1a1a1a;">{html.escape(line)}</p>'
            )
        ...
        print(f"Warning: Failed to load earnings summary from {earnings_summary_file}: {e}")
        return ""

def inject_custom_analysis_sections(html_content: str, report_data: dict) -> str:
    if not html_content:
        return html_content

    sec_html = build_sec_filing_highlights_html(report_data.get("sec_filing_analysis", {}))
    revenue_html = build_revenue_driver_analysis_html(report_data.get("revenue_driver_analysis", {}))
    earnings_html = report_data.get("earnings_analysis_html", "")

    updated_html = html_content

    if earnings_html and "Earnings Analysis" not in updated_html:
        updated_html, count = re.subn(
            r'(<h3 class="section-label">SEC Filing Highlights</h3>)',
            earnings_html + r'\1',
            updated_html,
            count=1
        )
        if count == 0:
            updated_html, count = re.subn(
                r'(<h3 class="section-label">Recent News Summary</h3>)',
                earnings_html + r'\1',
                updated_html,
                count=1
            )
        if count == 0:
            updated_html = updated_html.replace("</section>", earnings_html + "</section>", 1)

    if sec_html and "SEC Filing Highlights" not in updated_html:
        updated_html, count = re.subn(
            r'(<h3 class="section-label">Recent News Summary</h3>)',
            sec_html + r'\1',
            updated_html,
        ...
        if count == 0:
            updated_html = updated_html.replace("</section>", sec_html + "</section>", 1)

    if revenue_html and "Revenue Driver Analysis" not in updated_html:
        updated_html, count = re.subn(
            r'(<h3 class="section-label">Risks</h3>)',
            revenue_html + r'\1',
            updated_html,
            count=1
        )
        if count == 0:
            updated_html = updated_html.replace("</section>", revenue_html + "</section>", 1)

    return updated_html
def load_credit_cashflow_metrics_from_csv(file_path: str) -> pd.DataFrame:
    """
    Load credit and cashflow metrics from FMP ratios CSV.

    Compatible with both older FMP columns:
        calendarYear
    and newer/stable FMP columns:
    ...
    try:
        if not file_path or not os.path.exists(file_path):
            print("Warning: Ratios CSV path does not exist.")
            return pd.DataFrame()

        df = pd.read_csv(file_path)

        if df.empty:
            print("Warning: Ratios CSV is empty.")
            return pd.DataFrame()

        # Normalize year column
        if "calendarYear" in df.columns:
            df["report_year"] = df["calendarYear"]
        elif "fiscalYear" in df.columns:
            df["report_year"] = df["fiscalYear"]
        elif "year" in df.columns:
            df["report_year"] = df["year"]
        elif "date" in df.columns:
            df["report_year"] = df["date"].astype(str).str[:4]
        else:
            print("Warning: No recognizable year column found in ratios CSV.")
            return pd.DataFrame()
        ...
        # Metric mapping: display name -> possible source columns
        metric_mapping = {
            "Current Ratio": ["currentRatio"],
            "Quick Ratio": ["quickRatio"],
            "Cash Ratio": ["cashRatio"],
            "Debt / Assets": ["debtToAssetsRatio"],
            "Debt / Equity": ["debtToEquityRatio"],
            "Debt / Capital": ["debtToCapitalRatio"],
            "Interest Coverage": ["interestCoverageRatio"],
            "Operating Cash Flow Ratio": ["operatingCashFlowRatio"],
            "Operating Cash Flow / Sales": ["operatingCashFlowSalesRatio"],
            "FCF / Operating Cash Flow": ["freeCashFlowOperatingCashFlowRatio"],
            "Debt Service Coverage": ["debtServiceCoverageRatio"],
            "Dividend Payout Ratio": ["dividendPayoutRatio"],
            "Dividend Yield": ["dividendYield", "dividendYieldPercentage"],
            "Enterprise Value Multiple": ["enterpriseValueMultiple"],
            "Price / Book": ["priceToBookRatio"],
            "Price / Sales": ["priceToSalesRatio"],
            "Price / FCF": ["priceToFreeCashFlowRatio"],
            "Price / Operating Cash Flow": ["priceToOperatingCashFlowRatio"],
        }

        def _first_existing_col(possible_cols):
            for col in possible_cols:
                if col in df.columns:
                    return col
            return None

        df = df.sort_values(by="report_year").reset_index(drop=True)
        year_cols = sorted(df["report_year"].unique())

        output_rows = []

        for display_name, possible_cols in metric_mapping.items():
            source_col = _first_existing_col(possible_cols)
            ...
            for year in year_cols:
                ...
                if year_data.empty:
                    ...
                    value = year_data.iloc[0].get(source_col)
                ...
                if pd.isna(value):
                    ...
                    try:
                        ...
                        # Format selected fields as percentages
                        if display_name in [
                            "Debt / Assets",
                            "Debt / Equity",
                            "Debt / Capital",
        ...
        if result_df.empty:
            print("Warning: No compatible credit/cashflow ratio columns found.")
            return pd.DataFrame()

        return result_df

    except Exception as e:
        print(f"Error loading Credit & Cashflow metrics from CSV: {e}")
        return pd.DataFrame()
...
def process_text_content(args, analysis_df, peer_ebitda_df, peer_ev_ebitda_df, openai_api_key):
    ...
    earnings_summary_file = getattr(args, "earnings_summary_file", None)
    if not earnings_summary_file and getattr(args, "analysis_csv", None):
        inferred_earnings_path = os.path.join(os.path.dirname(args.analysis_csv), "earnings_summary.md")
        if os.path.exists(inferred_earnings_path):
            earnings_summary_file = inferred_earnings_path

    earnings_html = build_earnings_analysis_html(earnings_summary_file)
    processed_texts["earnings_analysis_html"] = earnings_html
    if earnings_html:
        print(f"✅ Earnings analysis loaded from {earnings_summary_file}")
    else:
        print("Warning: Earnings analysis summary not available; skipping Earnings Analysis section.")
...
def main():
    ...
    parser.add_argument("--earnings-summary-file", type=str, required=False, default=None, help="Path to earnings_summary.md for the earnings analysis section (optional).")
    ...
    parser.add_argument("--sec-filing-analysis-file", type=str, default=None, help="Path to SEC filing analysis JSON file.")
    parser.add_argument("--revenue-driver-analysis-file", type=str, default=None, help="Path to revenue driver analysis JSON file.")
    ...
    week_52_range = get_value(None, '52_range', 'N/A')
    ...
    earnings_analysis_html = processed_texts.get("earnings_analysis_html", "")

    # Directly append Earnings Analysis to the Investment Update section.
    # This is more reliable than template-level injection because all report templates already render investment_overview.
    if earnings_analysis_html and "Earnings Analysis" not in investment_overview_text:
        investment_overview_text = investment_overview_text + earnings_analysis_html
        print("✅ Earnings Analysis appended after Investment Update")
    elif earnings_analysis_html:
        print("ℹ️ Earnings Analysis already present in Investment Update")
    ...
    report_data = {
        ...
        "earnings_analysis_html": earnings_analysis_html,
    ...
    sec_filing_path = args.sec_filing_analysis_file
    if not sec_filing_path:
        candidate_path = os.path.join(os.path.dirname(args.analysis_csv), "sec_filing_analysis.json")
        if os.path.exists(candidate_path):
            sec_filing_path = candidate_path

    if sec_filing_path and os.path.exists(sec_filing_path):
        print(f"Loading SEC filing analysis from {sec_filing_path}...")
        report_data["sec_filing_analysis"] = load_json_file(sec_filing_path)
        if report_data["sec_filing_analysis"]:
            print("✅ SEC filing analysis loaded")
        else:
            print("⚠️ SEC filing analysis could not be loaded")

    revenue_driver_path = args.revenue_driver_analysis_file
    if not revenue_driver_path:
        candidate_path = os.path.join(os.path.dirname(args.analysis_csv), "revenue_driver_analysis.json")
        if os.path.exists(candidate_path):
            revenue_driver_path = candidate_path

    if revenue_driver_path and os.path.exists(revenue_driver_path):
        print(f"Loading revenue driver analysis from {revenue_driver_path}...")
        report_data["revenue_driver_analysis"] = load_json_file(revenue_driver_path)
        if report_data["revenue_driver_analysis"]:
            print("✅ Revenue driver analysis loaded")
        else:
            print("⚠️ Revenue driver analysis could not be loaded")
    ...
    professional_html_content = inject_custom_analysis_sections(professional_html_content, report_data)
    ...
    combined_html_content = inject_custom_analysis_sections(combined_html_content, report_data)
    ...
    for page_num, template in enumerate(templates, 1):
        ...
        html_content = inject_custom_analysis_sections(html_content, report_data)
